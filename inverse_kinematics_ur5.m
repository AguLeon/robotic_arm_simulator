%% If testing this file directly, specify desiredPos and eulAngles here and uncomment this section
...
%This solution is based on Andersen (2018), see report for further details.

function ik = inverse_kinematics_ur5(d, a, alpha, desiredPos, eulAngles, qInitial)

%% Check of parameters validity
    totalReach = sum(abs(d)) + sum(abs(a));
    if norm(desiredPos) > totalReach
        error('The specified desired position is out of reach. Try again.', totalReach);
    end
...
% R06 = eye(3);
% R06 = eul2rotm(eul);
% end_effector_adjust = -a_6*R06(:,1);
% disp(R06)
% pos = (pos + end_effector_adjust');
% disp(pos)


% Finding end effector related to base.
T06 = eye(4); 
T06(1:3,1:3) = eul2rotm(eulAngles); 
T06(1:3,4) = desiredPos;
...
for i = 1:length(theta1_)
    ...
    if acosValue > 1 %If this happens all the time, maybe just set theta5 to zero.
        error('Theta5 can not be determined. Failed to compute Inverse Kinematics')
    end
    for sign = [1 -1]
...
solution = closestSolution(solutions, qInitial);
...
function solution = closestSolution(solutions, q)
weights = [6 5 4 3 2 1];
